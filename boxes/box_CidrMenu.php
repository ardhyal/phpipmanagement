...
if ($_SESSION['access_level'] != "Administrator") {
    $sql = $connect->query("SELECT * FROM `NetMenu` WHERE `groupid` = '$_SESSION[groupid]' ORDER BY `NetMenuCidr` +0 ASC");
} else {
    $sql = $connect->query("SELECT * FROM `NetMenu` ORDER BY `NetMenuCidr` +0 ASC");
}
// print_r($sql);
while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
  echo $row;
    ...
    $sqlJoin = $connect->query("SELECT * FROM `net_ips` WHERE `view` = '1' AND `NetCidr` = '$row[NetMenuId]' ORDER BY `AddressId` +0 ASC");
    while ($rowJ = mysqli_fetch_array($sqlJoin)) {
