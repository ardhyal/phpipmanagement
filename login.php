...
// stripslashes on data
                ...
                // var_dump($query);
                // var_dump($connect);
                $check = $connect->query("SELECT * FROM `users`");
                if (mysqli_num_rows($check) == 1) {
                    while ($row = mysqli_fetch_array($check)) {
                        $LoginType = $row[type];
                        switch ($LoginType) {
                            case "local":
                                $val = $connect->query("SELECT * FROM `users` WHERE `type` = 'local' AND
                            ...
                            case "ldap":
                                $sql = $connect->query("SELECT * FROM `ldap` ORDER BY `ldapId`");
                                if (mysqli_num_rows($sql) == 0) {
                                    ...
                                    if ($ds) {
                                        ...
                                        if ($r == "true") {
                                            ...
                                            ldap_close($ds);
                                            $val = $connect->query("SELECT * FROM `users` WHERE `username` = '$username' AND
                                                                `type` = 'ldap'");
                    ...
                    if (mysqli_num_rows($val) == 1) {
                        ...
                        // add version
                        $version_defined = $connect->query("SELECT * FROM `version` ");
                        while ($row = mysqli_fetch_array($version_defined)) {
                            $_SESSION['version'] = $row[phpip];
                        }
                        // add preference
                        $pref_defined = $connect->query("SELECT * FROM `preference` WHERE `uid` = '$uid'");
                        if (mysqli_num_rows($pref_defined) == 1) {
                            while ($row = mysqli_fetch_array($pref_defined)) {
